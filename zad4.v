...
Proof.
 intros H. simpl. trivial.
Qed.

Lemma DN : forall x : B, Not(Not x) = x.
Proof.
intros x. destruct x.
-simpl. reflexivity.
-simpl. reflexivity.
...
Lemma DeMorgan : forall (x y : list B), AndL (NotL x) (NotL y) = NotL(OrL x y).
Proof.
Admitted.
...
Proof.
simpl. intros H. 
assert(DM: forall (x y : list B), AndL (NotL x) (NotL y) = NotL(OrL x y)).
{
 intros x0 y0. induction x0.
  -simpl. reflexivity.
  -rewrite DeMorgan. reflexivity.
}
rewrite DeMorgan.
assert(G: forall (x : list B), NotL(NotL x) = x).
{
 intros x0. induction x0.
  -simpl. reflexivity.
  -simpl. rewrite IHx0. rewrite DN. reflexivity.
}
